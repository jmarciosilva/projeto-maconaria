...
- **Configuração de e-mail é um singleton** (`ConfiguracaoEmail::atual()`), no mesmo padrão de `ConfiguracaoInstitucional` — um único registro (`configuracoes_email`) para host, porta, usuário, senha, criptografia, remetente e uma flag `ativo`.
- **`ConfiguracaoEmail::atual()` passa os defaults explicitamente no `create()`** (`mailer: log`, `ativo: false`), em vez de confiar apenas no `default()` da coluna na migration. Motivo: `create([])` não popula os atributos em memória da instância recém-criada com os defaults do banco — só uma releitura (`fresh()`/`refresh()`) traria esses valores. Sem o `create()` explícito, o primeiro acesso a `->mailer` logo após a criação do singleton retornava `null` em vez de `MailerEmail::LOG`, quebrando a view com "Attempt to read property on null". Mesma categoria do bug já corrigido em `ConfiguracaoInstitucional` (ver seção de CMS acima), mas com uma causa raiz diferente.
- **Senha criptografada com o cast nativo `encrypted` do Eloquent** (Crypt/`APP_KEY`), sem reinventar criptografia própria. A coluna nunca é comparável/pesquisável (não há necessidade de `WHERE senha = ...`), então não há a mesma restrição que levou a não criptografar o CPF do Irmão.
- **Senha nunca é exposta**: o Model tem `$hidden = ['senha']` (nunca aparece em `toArray()`/`toJson()`), a view de edição nunca renderiza o valor atual (apenas um aviso "senha já configurada" quando há uma), e a auditoria (`RegistradorDeAuditoria`) grava explicitamente apenas os campos não sensíveis (`mailer`, `host`, `porta`, `usuario`, `criptografia`, `remetente_nome`, `remetente_email`, `ativo`) — nunca o campo `senha`, nem antes nem depois da alteração.
- **Campo de senha em branco mantém a senha já cadastrada**: o formulário nunca exige reenviar a senha a cada edição; `ConfiguracaoEmailController::update()` só sobrescreve `senha` quando o campo vem preenchido.
- **Aplicação dinâmica via `AplicadorConfiguracaoEmail`**: chamada em `AppServiceProvider::boot()` a cada requisição, sobrepõe `config('mail.*')` com os dados salvos no painel, mas só quando `ativo = true` — enquanto o SMTP não for configurado e ativado, o mailer padrão do `.env` (`log`) continua valendo, para nunca travar o envio de e-mails do sistema com dados incompletos. Tem uma guarda explícita (`Schema::hasTable`) porque o `boot()` roda antes de qualquer migration em uma instalação nova (e no próprio `php artisan migrate` inicial) — sem essa guarda, a aplicação inteira quebraria por falta da tabela. A mesma aplicação é chamada novamente, de forma explícita, no início do envio do e-mail de teste — garante que o teste sempre reflita a configuração recém-salva, mesmo que o boot desta mesma requisição já tivesse ocorrido antes da gravação.
