...
- Upload de arquivos (fotos, documentos): validação de MIME real (não apenas extensão), dimensões e armazenamento fora da pasta pública para documentos privados. Fase 9 implementou anexos privados do módulo de Documentos e Trabalhos no disco `local`; Fase 10 implementou imagens da Galeria no disco `public`, com visibilidade controlada por status/visibilidade do álbum.
